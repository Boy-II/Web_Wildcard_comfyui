...
• 批量遷移分類：將一批 wildcard 移到正確分類（使用 move_wildcard action）
...
將 wildcard 遷移到其他分類（需要 wildcard ID 和目標分類 ID）：
<action>{{"type":"move_wildcard","id":<wildcardID>,"category_id":<目標分類ID>}}</action>
...
• 使用者確認後才輸出 <action> 標籤
• 一次回覆可包含多個 <action> 標籤（可批量 move_wildcard）
• <action> 標籤不會顯示給使用者，只是系統執行用

當訊息中附有「[分類 wildcard 清單]」或「[相關 wildcard]」區塊時，wildcard 資料包含 ID，可直接用於操作。
若清單顯示「尚有 N 筆未載入」，使用者可繼續說「繼續下一批」來處理剩餘的。
...
def _get_wildcards_by_category(message: str, limit: int = 100, offset: int = 0) -> dict | None:
    """
    Detect a category name/display_name mention in message and bulk-fetch its wildcards.
    Returns dict with keys: category, wildcards, total, offset, remaining.
    Returns None if no matching category found in message.
    """
    from webapp.models import Category, Wildcard

    cats = Category.query.order_by(Category.level).all()

    matched_cat = None
    # Check for "繼續下一批" continuation pattern — caller passes offset via message parsing
    # Primary: find category by display_name or name appearing in message
    for cat in cats:
        names_to_check = []
        if cat.display_name:
            # Strip numeric prefix like "0.分類名" → "分類名"
            stripped = re.sub(r'^\d+\.', '', cat.display_name).strip()
            names_to_check.append(stripped)
            names_to_check.append(cat.display_name)
        if cat.name:
            names_to_check.append(cat.name)
    ...
    total = Wildcard.query.filter_by(category_id=matched_cat.id, is_active=True).count()
    wildcards = (
        Wildcard.query
        .filter_by(category_id=matched_cat.id, is_active=True)
        .order_by(Wildcard.content)
        .offset(offset)
        .limit(limit)
        .all()
    ...
    return {
        ...
        'wildcards': [
            {
                ...
                'content': w.content,
                'content_zh': w.content_zh or '',
                'category': matched_cat.display_name,
        ...
        'total': total,
        'offset': offset,
        'remaining': max(0, total - offset - len(wildcards)),
    }
...
def _execute_actions(text: str) -> tuple[str, list[str]]:
    ...
    def _run(match):
        ...
        try:
            ...
            elif atype == 'move_wildcard':
                from webapp.models import db, Wildcard, Category
                w = Wildcard.query.get(data.get('id'))
                if not w:
                    actions_log.append(f'❌ 找不到 wildcard ID={data.get("id")}')
                    return ''
                new_cat = Category.query.get(data.get('category_id'))
                if not new_cat:
                    actions_log.append(f'❌ 找不到目標分類 ID={data.get("category_id")}')
                    return ''
                old_name = w.category.display_name if w.category else '?'
                w.category_id = new_cat.id
                db.session.commit()
                actions_log.append(f'✅ 已將「{w.content}」從「{old_name}」移至「{new_cat.display_name}」')
...
def chat(message: str, history: list[dict], image: str = None, offset: int = 0) -> dict:
    """
    image: base64 data URL string, e.g. 'data:image/jpeg;base64,...'
    offset: for pagination when user says "繼續下一批"
    """
    profile = get_profile()
    if not profile:
        return {
    ...
    # Detect "繼續下一批" — extract offset from last assistant context if present
    is_continuation = '繼續' in message and ('下一批' in message or '下批' in message)

    # Try category-based bulk fetch first; fall back to keyword search
    cat_result = _get_wildcards_by_category(message, limit=100, offset=offset)

    system_prompt = SYSTEM_PROMPT_TEMPLATE.format(category_list=_get_categories_text())

    user_text = message
    if cat_result:
        wildcards = cat_result['wildcards']
        remaining = cat_result['remaining']
        cat = cat_result['category']
        wc_lines = '\n'.join(
            f'  - [ID:{w["id"]}] {w["content"]}（{w["content_zh"]}）[分類ID:{w["category_id"]}]'
            for w in wildcards
        )
        footer = f'\n尚有 {remaining} 筆未載入' if remaining > 0 else ''
        user_text = (
            f'{message}\n\n'
            f'[分類 wildcard 清單：{cat.display_name}（ID:{cat.id}），共 {cat_result["total"]} 筆，本批 {len(wildcards)} 筆{footer}]\n'
            f'{wc_lines}'
        )
    else:
    ...
    result = {'reply': reply, 'wildcards': wildcards, 'actions': actions_log}
    if cat_result:
        result['category_batch'] = {
            'category_id': cat_result['category'].id,
            'category_name': cat_result['category'].display_name,
            'total': cat_result['total'],
            'offset': cat_result['offset'],
            'remaining': cat_result['remaining'],
        }
    return result
